...
fun checkTypeMismatch(
    ...
    val rValueType = rValue.typeRef.coneTypeSafe<ConeKotlinType>() ?: return
    ...
    if (!isSubtypeForTypeMismatch(typeContext, subtype = rValueType, supertype = lValueType)) {
        ...
        if (rValue.isNullLiteral && lValueType.nullability == ConeNullability.NOT_NULL) {
            ...
            val diagnostics = when {
                initializer ->
                    FirErrors.INITIALIZER_TYPE_MISMATCH.on(source, lValueType, rValueType)
                source.kind is FirFakeSourceElementKind.DesugaredIncrementOrDecrement ->
                    FirErrors.RESULT_TYPE_MISMATCH.on(source, lValueType, rValueType)
                else ->
                    FirErrors.ASSIGNMENT_TYPE_MISMATCH.on(source, lValueType, rValueType)
            }
